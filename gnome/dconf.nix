...
with lib.hm.gvariant; {
  dconf.settings =
    ...
    // (let
      ...
      keyBindings = lib.listToAttrs (map ({name, ...} @ keyBinding: {
          name = "${basePath}/custom-keybindings/${name}";
          value = keyBinding;
        })
        customKeyBindings);
      bindingsList = map ({name, ...}: "/${basePath}/custom-keybindings/${name}/") customKeyBindings;
    in
      {
        ${basePath} = {
